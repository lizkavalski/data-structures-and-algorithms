...
const oddValues = (arr) => {
 ...
 let oddnum = arr.filter(arr =>arr % 2);
  return (oddnum);
};
...
const filterStringsWithVowels = (arr) => {
    let vowels =arr.filter(arr=> arr.match(/[a|e|i|o|u]/g));
    return (vowels);
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let end = forbiddenValue.filer(arr => !forbiddenValues === arr);
  return (end);
};
...
const getCharactersWithoutChildren = (arr) => {
  let children = characters.filter( character => !character.children);
